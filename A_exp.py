...
"""
fig,ax4 = plt.subplots(figsize=(3,2.5))
...
for i,(data,rho,gamma,mu) in enumerate(drg[:]): #x[1:2] is [x[1]], an array
    ...
    log10a, b,sigmalog10a, sigmab  = leastsq_weighted(np.log10(Ca),np.log10(1e-6*hflat),log10uerr,log10herr)
    a = 10**(log10a)
    berr = sigmab
    ...
    print 'a = %.8f+/-%.8f' %(a,aerr),'for mu',mu,'cP'
    ...
    #ax.set_xlabel(r'$log(U)$',fontsize=18)
    #ax.set_ylabel(r'$log$(thin part thickness)',fontsize=18)
    #ax.legend()
...
ax4.yaxis.set_label_coords(-0.3,0.52)
ax4.set_xticks([1.5,2.5])
ax4.set_yticks([-5.5,-4.7])
