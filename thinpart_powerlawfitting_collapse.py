...
for i,(data,rho,gamma,mu) in enumerate(drg[:]): #x[1:2] is [x[1]], an array
    ...
    ax.errorbar(1e6*(np.sqrt(gamma/(rho*9.8))*np.power(etha_air*u*1e-3/gamma,0.4621)*(etha_air/(mu*1e-3))**0.4313),hflat,fmt=markerlist[i],ms=8,mew=1,mfc=cmap(i),ecolor='C0',xerr=0, yerr=0,capsize=4,label='%scP'%mu,zorder=1)
    #plt.plot(1e-6*hflat,10*np.sqrt(gamma/(rho*9.8))*np.power(1e-3*u*etha_air/gamma, 0.4782)*(etha_air/(mu*1e-3))**(0.4449),color=cmap(i),linestyle='-',linewidth=1,zorder=3)
    print 0.00008/(np.sqrt(gamma/(rho*9.8))*np.power(etha_air/gamma,0.46)*(etha_air*1e3)**0.43)
    #ax.legend()
...
ax.plot(np.arange(0,4),8.2*np.arange(0,4))

ax.set_xlabel(r'$\kappa^{-1}Ca^{0.46}(\eta_{in}/\eta_{out})^{0.43}$',fontsize=23,labelpad=0)
ax.set_ylabel(r'$H_{thin}(\mu m)$',fontsize=24,labelpad=0)
#ax.set_yticks(1e-6*np.arange(0,25,5))
#ax.set_yticklabels([repr(i) for i in np.arange(0,25,5)])
#ax.set_xticks(1e-6*np.arange(0,25,5))
#ax.set_xticklabels([repr(i) for i in np.arange(0,25,5)])
plt.tick_params(labelsize=18,right=True,top=True)
#plt.axis('equal')
ax.set_xlim(0,2.4)
ax.set_ylim(0,2.4*8.2)
plt.show()
