...
from leastsq import leastsq_weighted_thru0 as myfit
...
def plotfunction(expo1,expo2,c):
    for i,(data,rho,gamma,mu) in enumerate(drg[:]):
        ...
        U = np.arange(0,10*np.max(u)*1e-3,0.001)
        etha = 18.27e-6 
        Ca = etha*U/gamma
        # y = p*x
        p, perr = myfit(u,1/np.cos(phi),du,dphi*np.sin(phi)/(np.cos(phi)**2))
        umax = 1/p
        umaxerr = perr/(p**2)
        ...
        #    continue
        #ax.errorbar(u,1/np.cos(phi),xerr=du,yerr=dphi*np.sin(phi)/(np.cos(phi)**2),fmt=markerlist[i],ms=8,markeredgewidth=1,mfc=cmap(i),ecolor='C0',capsize=4,alpha=1,label='%scP'%mu,zorder=1)
        #ax.set_xlim(0,950)
        #ax.set_ylim(0,5)
        #ax.legend(loc='upper right',ncol=1,framealpha=1)
        #ax.plot(1e3*U,p*1e3*U,color=cmap(i/20),zorder=0)
        ...
        ax.set_xlabel(r'$log(\eta_{liq})$',fontsize=18)
        ax.set_ylabel(r'$log(U_{max})$',fontsize=18)
...
#ax.set_ylabel(r'$\frac{1}{\cos(\phi)}$',fontsize=20)
