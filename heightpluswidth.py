...
def plotfunction(expo1,expo2,c):
    for i,(data,rho,gamma,mu) in enumerate(drg[:]):
        ...
        ax.errorbar(u,(thickw+2*height/np.tan(phi))/W,fmt=markerlist[i],ms=8,mew=1,mfc=cmap(i),ecolor='C0',xerr=0, yerr=0,label='%scP'%mu)#for legend
...
#ax.set_xlim(0.4,1)
#ax.set_ylim(0.4,1)
#ax.plot(np.arange(0.4,2,0.1),np.arange(0.4,2,0.1))
ax.set_xlabel('$U (mm/s)$',fontsize=24,labelpad=0)
ax.set_ylabel(r'$W+2Lcot\phi$',fontsize=24,labelpad=0)
plt.show()
